...
// strspn()返回str起始部分group中任意字符的字符数。
...
int setnonblocking(int fd){
    ...
    int newp = old | O_NONBLOCK;
    fcntl(fd, F_SETFL, newp);
    return old;
}
...
char *root, int TRIGMode,int close_log, string user, 
...
bool http_conn::read_once(){
    ...
    if(0 == m_TRIGMode){
        ...
        while(true){
            bytes_read = recv(m_sockfd, m_read_buf + m_read_idx, READ_BUFFER_SIZE - m_read_idx, 0);
            if(bytes_read == -1){
                // 网络事件，EWOULDBLOCK：用于非阻塞模式，不需要重新读或者写
