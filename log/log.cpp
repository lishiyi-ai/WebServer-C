...
bool Log::init(const char *file_name, int close_log, int log_bus_size, 
int split_lines, int max_queue_size){
    ...
    if(p == NULL){
        ...
        snprintf(log_full_name, 512, "%s%d_%02d_%02d_%s", dir_name, 
...
void Log::write_log(int level, const char *format, ...){
    ...
    if(m_today != my_tm.tm_mday || m_count % m_split_lines == 0){
        ...
        snprintf(tail, 32, "%d_%02d_%02d_", my_tm.tm_year + 1900,
        ...
        {
            snprintf(new_log, 512, "%s%s%s.%lld", dir_name, tail, log_name, m_count / m_split_lines);
        }
