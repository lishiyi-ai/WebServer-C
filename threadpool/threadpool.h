...
public:
    ...
    threadpool(int actor_model, connection_pool *connPool,
                int thread_number = 8, int max_request = 10);
    virtual ~threadpool();
    bool append(T *reques, int state);
...
private:
    ...
    locker m_queuelocker; // 保护请求队列的和互斥锁
...
int thread_number, int max_requests):m_actor_model(actor_model),m_thread_number(thread_number)
